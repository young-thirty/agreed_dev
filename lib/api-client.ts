...
/** OAuth 링크와 이미지 URL도 같은 FastAPI 주소를 쓰게 하는 공개 헬퍼다. */
export function apiUrl(path: string): string {
  if (!API_BASE_URL) {
    throw new Error('NEXT_PUBLIC_API_BASE_URL이 설정되지 않았습니다. 배포 환경변수를 확인해 주세요.');
  }
...
function parseApiResult<T>(payload: unknown, status: number): ApiResult<T> {
  ...
  return { ok: false, error: '서버 응답 형식을 확인하지 못했습니다. 잠시 후 다시 시도해 주세요.' };
}
