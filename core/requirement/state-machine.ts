 ...
 * LLM이 제안할 수 있는 상태.
 *
 * '합의'와 '완료'는 빠져 있다. 합의 성립과 완료 처리는 사람이 결정하는 사실 확인이지
 * 추론의 대상이 아니기 때문이다.
 */
export const LLM_PROPOSABLE: readonly RequirementStatus[] = [
  '미확정',
  '문의',
  '요청',
  '제안',
 ...
 * 여기서는 두 가지만 본다. LLM이 제안해도 되는 값인가, 그리고 전이표상 도달 가능한가.
 *
 * 둘 중 하나라도 아니면 거부하지 않고 '미확정'으로 내린다. 거부하면 화면에 아무것도
 * 남지 않지만, 강등하면 사람이 판단할 거리가 남기 때문이다.
 */
export function demote(from: RequirementStatus, proposed: RequirementStatus): RequirementStatus {
  if (!LLM_PROPOSABLE.includes(proposed)) return '미확정';
